            ...
            <div className="hidden lg:block border-t border-gray-200">
                <nav className="container mx-auto px-6 flex items-center justify-center space-x-10 text-sm font-bold h-12">
                    ...
                    <div className="relative h-full flex items-center" onMouseEnter={() => setIsProductsMenuOpen(true)} onMouseLeave={() => setIsProductsMenuOpen(false)}>
                        <button className={`flex items-center transition-colors duration-300 ${isProductsMenuOpen ? 'text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}>Ürünler <ChevronDownIcon className={`transition-transform duration-300 ${isProductsMenuOpen ? 'rotate-180' : ''}`}/></button>
                        <AnimatePresence>
                            {isProductsMenuOpen && (
