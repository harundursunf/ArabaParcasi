...
// --- İkonlar ---
const ProfileIcon = () => <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>;
            ...
            <div className="container mx-auto px-4 sm:px-6">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <Link to="/profile" className="relative flex items-center p-2 text-gray-600 hover:text-yellow-500 transition-colors">
                            <ProfileIcon />
                        </Link>
                    </div>
                </div>
                        ...
                        <AnimatePresence>
                            {isProductsMenuOpen && ( 
                                <motion.div 
                                    ...
                                    className="absolute top-full left-1/2 -translate-x-1/2 w-screen max-w-4xl bg-white shadow-lg rounded-b-lg border-t border-gray-100"
                                >
                                    <div className="grid grid-cols-4 gap-8 py-8 px-6">
                                        <div className="col-span-3 grid grid-cols-3 gap-8">
                                            <div>
                                                <h3 className="font-bold text-gray-500 uppercase text-xs tracking-wider mb-4">Kategoriler</h3>
                                                <div className="space-y-3">
                                                    {kategoriler.slice(1).map(kategori => (
                                                        <Link key={kategori} to="/urunler" onClick={() => setIsProductsMenuOpen(false)} className="block text-gray-600 hover:text-yellow-500 whitespace-nowrap">{kategori}</Link>
                                                    ))}
                                                </div>
                                            </div>
                                            <div>
                                                <h3 className="font-bold text-gray-500 uppercase text-xs tracking-wider mb-4">Markalar</h3>
                                                <div className="space-y-3">
                                                    {markalar.slice(0, 5).map(marka => (
                                                        <Link key={marka} to="/urunler" onClick={() => setIsProductsMenuOpen(false)} className="block text-gray-600 hover:text-yellow-500">{marka}</Link>
                                                    ))}
                                                </div>
                                            </div>
                                        </div>
                                        <div className="col-span-1">
                                            <div className="bg-gray-100 rounded-lg h-full w-full overflow-hidden">
                                                <img src="/x1.avif" alt="promosyon" className="w-full h-full object-cover"/>
                                            </div>
                                        </div>
                                    </div>
                                </motion.div>
                        ...
                        >
                            <div className="p-6 flex flex-col h-full">
                                <div className="flex justify-end items-center mb-8">
                                    <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 -mr-2"><CloseIcon /></button>
                                </div>
                                <div className="mb-8">
                                    <form onSubmit={handleSearchSubmit} className="relative">
